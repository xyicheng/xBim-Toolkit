...
// Filename:    SelectionService3D.cs
// Published:   01, 2012
...
{
    internal class SelectionService3D
    {
        private ModelVisual3D _host;
        ...
        public SelectionService3D(ModelVisual3D host)
        {
            _host = host;
        }
        ...
        {
            ClearSelection();
            CurrentSelection.AddRange(_host.Children.OfType<ISelectable>());
            CurrentSelection.ForEach(item => item.IsSelected = true);
        }

        internal List<IGroupable> GetGroupMembers(IGroupable item)
        {
            IEnumerable<IGroupable> list = _host.Children.OfType<IGroupable>();
