...
namespace Xbim.DynamicGrouping
{
    public class GroupingByXml
    {
        ...
        public GroupingByXml(IModel model)
        {
            _model = model;
        }
        ...
        public bool GroupElements(string XMLfileName, IfcGroup rootGroup)
        {
            ...
            {
                _rootGroup = _model.New<IfcGroup>(g => g.Name = "Root group");
            }


            using (Transaction trans = _model.BeginTransaction("Elements to groups"))
            {
        ...
        public bool PerformGrouping()
        {
            ...
            if (noGroup == null)
            {
                noGroup = _model.New<IfcGroup>(gr => { gr.Name = "No group"; gr.Description = ""; });
                _rootGroup.AddObjectToGroup(noGroup);
            }
            List<IfcElement> allElements = _model.InstancesOfType<IfcElement>().ToList();
            List<IfcTypeObject> allTypes = _model.InstancesOfType<IfcTypeObject>().ToList();
            ...
            foreach (XmlNode group in groups)
            {
                ...
                if (iGroup == null)
                {
                    iGroup = _model.New<IfcGroup>(gr => gr.Name = grpName);
                ...
                foreach (XmlNode element in group)
                {
                    ...
                    foreach (XmlNode elTypeNode in elTypeNodeList) //there should be just one 'elTypeNode'
                    {
                        ...
                        elemType = GetIfcType(elTypeName);
                        if (elemType == null) continue;
                        ...
                        types = _model.Instances.Where(typeQueryBuilder.BuildQuery());
                        break;
                    }

                    //get elements and element type
                    IEnumerable<IPersistIfcEntity> instances = allElements.Where(qBuilder.BuildQuery());
                    ...
                    foreach (var inst in instances)
                    {
                        ...
                        if (el != null && allElements.Remove(el))
                        {
                            ...
                            IEnumerable<IfcRelDefinesByType> rels = _model.InstancesWhere<IfcRelDefinesByType>(r => r.RelatingType == type);
        ...
        /// <summary>
        /// Gets IFC type from static dictionary of IFC types.
        /// Search is case insensitive
        /// </summary>
        /// <param name="typeName">Name of the type (case insensitive)</param>
        /// <returns>System.Type of the IFC type</returns>
        public Type GetIfcType(string typeName)
        {
            typeName = typeName.ToUpper();
            Dictionary<string, IfcType> types = IfcInstances.IfcTypeLookup;
            IfcType t = null;
            ...
            errLog.WriteLine("'" + eName + "' data type does not exist or is not implemented.");
            return null;
        }
