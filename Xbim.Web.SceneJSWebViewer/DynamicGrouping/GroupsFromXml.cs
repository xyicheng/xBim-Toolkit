...
namespace Xbim.DynamicGrouping
{
    public class GroupsFromXml
    {
        ...
        public GroupsFromXml(IModel model)
        {
        ...
        {
            _xmlDoc = xmlDocument;
            using (Transaction trans = _model.BeginTransaction("Groups creation"))
            {
                Process();
                trans.Commit();
            }
        ...
        public IfcGroup GetExistingRootGroup(XmlDocument xmlDocument)
        {
            ...
            foreach (XmlNode group in groups)
            {
                ...
                IfcGroup root = _model.InstancesWhere<IfcGroup>(g => g.Name == className).FirstOrDefault();
                if (root != null)
                {
                    return root;
                }
        ...
        private void Process()
        {
            ...
            using (Transaction transaction = _model.BeginTransaction("Groups creation"))
            {
        ...
        {
            IfcGroup group = _model.InstancesWhere<IfcGroup>(g => g.Name == groupName).FirstOrDefault();
            if (group == null) group = _model.New<IfcGroup>(g => { g.Name = groupName; g.Description = classification; });
