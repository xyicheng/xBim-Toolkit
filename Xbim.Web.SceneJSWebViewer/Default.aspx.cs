...
namespace Xbim.SceneJSWebViewer
{
    public partial class _Default : System.Web.UI.Page
    {
        ...
        private static void CreateLayeredElementTypes(IModel model)
        {
            ...
            IEnumerable<IfcBuildingElement> elements = model.InstancesWhere<IfcBuildingElement>(el =>
        ...
        {
            if (element is IfcBuildingElementProxy) return model.New<IfcBuildingElementProxyType>();
            if (element is IfcCovering) return model.New<IfcCoveringType>();
            if (element is IfcBeam) return model.New<IfcBeamType>();
            if (element is IfcColumn) return model.New<IfcColumnType>();
            if (element is IfcCurtainWall) return model.New<IfcCurtainWallType>();
            if (element is IfcDoor) return model.New<IfcDoorStyle>();
            if (element is IfcMember) return model.New<IfcMemberType>();
            if (element is IfcRailing) return model.New<IfcRailingType>();
            if (element is IfcRampFlight) return model.New<IfcRampFlightType>();
            if (element is IfcWall) return model.New<IfcWallType>();
            if (element is IfcSlab) return model.New<IfcSlabType>(t => t.PredefinedType = (element as IfcSlab).PredefinedType ?? IfcSlabTypeEnum.NOTDEFINED);
            if (element is IfcStairFlight) return model.New<IfcStairFlightType>();
            if (element is IfcWindow) return model.New<IfcWindowStyle>();
            if (element is IfcPlate) return model.New<IfcPlateType>();
            if (element is IfcCovering) return model.New<IfcCoveringType>();
            if (element is IfcRoof) return model.New<IfcSlabType>(sl => sl.PredefinedType = IfcSlabTypeEnum.ROOF);

            return model.New<IfcBuildingElementProxyType>();
        }
        ...
        private static void CreateElementTypesByName(IModel model)
        {
            Xbim.XbimExtensions.Transactions.Transaction trans = model.BeginTransaction("Create element types by name.");
            //get elements without element type
            IEnumerable<IfcBuildingElement> elements = model.InstancesWhere<IfcBuildingElement>(el =>
                el.GetDefiningType() == null);
        ...
        public static string AddClassification(string xbimFilename)
        {
            try
            {
                ...
                model.Export(XbimStorageType.IFC, Path.ChangeExtension(file, "ifc"));
