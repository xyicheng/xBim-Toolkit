        ...
        public override COBieSheet<COBieConnectionRow> Fill()
        {
            ...
            IEnumerable<IfcRelConnectsElements> ifcRelConnectsElements = Model.InstancesOfType<IfcRelConnectsElements>()
                                                                        .Where(rce => rce.RelatedElement != null && 
                                                                                      !ComponentExcludeTypes.Contains(rce.RelatedElement.GetType()) &&
                                                                                      rce.RelatingElement != null &&
                                                                                      !ComponentExcludeTypes.Contains(rce.RelatingElement.GetType())
                                                                                      );
            ...
            foreach (IfcRelConnectsElements ifcRelConnectsElement in ifcRelConnectsElements)
            {
                ...
                ////we need to filter out where we do not have a connection to the components tab
                //if ((relatingElement == null) ||
                //    (relatedElement == null) ||
                //    (ComponentExcludeTypes.Contains(relatingElement.GetType())) ||
                //    (ComponentExcludeTypes.Contains(relatedElement.GetType()))
                //    )
                //    continue;
                ...
                conn.SheetName = GetSheetByObjectType(relatingElement);
                ...
                foreach (IfcRelConnectsPorts port  in ifcRelConnectsPortsElement)
	            {
                    ...
                    if ((port.RealizingElement != null) && (!string.IsNullOrEmpty(port.RealizingElement.ToString())) && (!realizingElement.Contains(port.RealizingElement.ToString())))
                        realizingElement.Add(port.RealizingElement.ToString());

                    if ((port.RelatedPort != null) && (!string.IsNullOrEmpty(port.RelatedPort.Name.ToString())) && (!relatedPort.Contains(port.RelatedPort.Name.ToString())))
                        relatedPort.Add(port.RelatedPort.Name.ToString());

                    if ((port.RelatingPort != null) && (!string.IsNullOrEmpty(port.RelatingPort.Name.ToString())) && (!relatingPort.Contains(port.RelatedPort.Name.ToString())))
                        relatingPort.Add(port.RelatingPort.Name.ToString());
	            }

                conn.RealizingElement = (realizingElement.Count > 0) ? string.Join( " : ", realizingElement) : DEFAULT_STRING ;
