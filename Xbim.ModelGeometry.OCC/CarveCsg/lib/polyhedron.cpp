...
namespace {
  bool emb_test(carve::poly::Polyhedron *poly,
                ...
                int m_id, double EPSILON, double EPSILON2) {

    std::map<int, carve::PointClass> result;
#if defined(CARVE_DEBUG)
    std::cerr << "test " << v << " (m_id:" << m_id << ")" << std::endl;
#endif
    poly->testVertexAgainstClosedManifolds(v, result, true,  EPSILON,  EPSILON2);
    std::set<int> inside;
...
namespace carve {
  namespace poly {
    ...
    bool Polyhedron::calcManifoldEmbedding(double EPSILON, double EPSILON2) {
      ...
      for (size_t i = 0; i < vertices.size(); ++i) {
        ...
        if (embedding.find(m_id) == embedding.end()) {
          if (emb_test(this, embedding, vertices[i].v, m_id,  EPSILON,  EPSILON2) && embedding.size() == MCOUNT) {
            carve::Timing::stop();
            goto done;
          }
        }
      ...
      for (size_t i = 0; i < edges.size(); ++i) {
        if (connectivity.edge_to_face[i].size() == 2) {
          ...
          if (embedding.find(m_id) == embedding.end()) {
            if (emb_test(this, embedding, (edges[i].v1->v + edges[i].v2->v) / 2, m_id,  EPSILON,  EPSILON2) && embedding.size() == MCOUNT) {
              carve::Timing::stop();
              goto done;
            }
          }
      ...
      for (size_t i = 0; i < faces.size(); ++i) {
        ...
        if (embedding.find(m_id) == embedding.end()) {
          carve::geom2d::P2 pv;
          if (!carve::geom2d::pickContainedPoint(faces[i].projectedVertices(), pv, EPSILON)) continue;
          carve::geom3d::Vector v = carve::poly::face::unproject(faces[i], pv);
          if (emb_test(this, embedding, v, m_id,  EPSILON,  EPSILON2) && embedding.size() == MCOUNT) {
            carve::Timing::stop();
            goto done;
          }
        }
    ...
    void Polyhedron::testVertexAgainstClosedManifolds(const carve::geom3d::Vector &v,
                                                      ...
                                                      bool ignore_orientation, double EPSILON, double EPSILON2) const {

      for (size_t i = 0; i < faces.size(); i++) {
        if (!manifold_is_closed[faces[i].manifold_id]) continue; // skip open manifolds
        if (faces[i].containsPoint(v, EPSILON,  EPSILON2)) {
      ...
      for (;;) {
        ...
        for (unsigned i = 0; !failed && i < possible_faces.size(); i++) {
          ...
          switch (possible_faces[i]->lineSegmentIntersection(line, intersection,EPSILON, EPSILON2)) {
          case INTERSECT_FACE: {
