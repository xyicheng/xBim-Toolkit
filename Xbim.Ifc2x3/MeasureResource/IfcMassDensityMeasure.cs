...
// Project:     Xbim.Ifc
// Filename:    IfcMassDensityMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcMassDensityMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcMassDensityMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcMassDensityMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcMassDensityMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcMassDensityMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcMassDensityMeasure) value).ToString());
            else
