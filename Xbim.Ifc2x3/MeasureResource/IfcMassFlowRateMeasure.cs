...
// Project:     Xbim.Ifc
// Filename:    IfcMassFlowRateMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcMassFlowRateMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcMassFlowRateMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcMassFlowRateMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcMassFlowRateMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcMassFlowRateMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcMassFlowRateMeasure) value).ToString());
            else
