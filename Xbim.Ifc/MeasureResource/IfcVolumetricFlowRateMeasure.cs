...
// Project:     Xbim.Ifc
// Filename:    IfcVolumetricFlowRateMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcVolumetricFlowRateMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcVolumetricFlowRateMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcVolumetricFlowRateMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcVolumetricFlowRateMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcVolumetricFlowRateMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcVolumetricFlowRateMeasure) value).ToString());
            else
