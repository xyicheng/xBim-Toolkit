...
// Project:     Xbim.Ifc
// Filename:    IfcKinematicViscosityMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcKinematicViscosityMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcKinematicViscosityMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcKinematicViscosityMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcKinematicViscosityMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcKinematicViscosityMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcKinematicViscosityMeasure) value).ToString());
            else
