...
{
    [Serializable]
    public struct IfcPowerMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcPowerMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcPowerMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcPowerMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcPowerMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcPowerMeasure) value).ToString());
            else
