...
// Project:     Xbim.Ifc
// Filename:    IfcModulusOfElasticityMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcModulusOfElasticityMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcModulusOfElasticityMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcModulusOfElasticityMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcModulusOfElasticityMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcModulusOfElasticityMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcModulusOfElasticityMeasure) value).ToString());
            else
