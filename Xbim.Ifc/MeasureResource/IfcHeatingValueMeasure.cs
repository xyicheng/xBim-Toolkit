...
// Project:     Xbim.Ifc
// Filename:    IfcHeatingValueMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcHeatingValueMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcHeatingValueMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcHeatingValueMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcHeatingValueMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcHeatingValueMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcHeatingValueMeasure) value).ToString());
            else
