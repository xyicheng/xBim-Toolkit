...
// Project:     Xbim.Ifc
// Filename:    IfcPlaneAngleMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcPlaneAngleMeasure : IPersistIfc, IfcMeasureValue, IfcOrientationSelect
    {
        ...
        {
            if (value.HasValue)
                return new IfcPlaneAngleMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcPlaneAngleMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcPlaneAngleMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcPlaneAngleMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcPlaneAngleMeasure) value).ToString());
            else
