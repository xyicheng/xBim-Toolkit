...
// Project:     Xbim.Ifc
// Filename:    IfcSpecificHeatCapacityMeasure.cs
// Published:   01, 2012
...
{
    [Serializable]
    public struct IfcSpecificHeatCapacityMeasure : IPersistIfc, IfcDerivedMeasureValue
    {
        ...
        {
            if (value.HasValue)
                return new IfcSpecificHeatCapacityMeasure((double) value);
            else
        ...
        public static explicit operator double(IfcSpecificHeatCapacityMeasure? obj)
        {
            if (obj.HasValue)
                return ((IfcSpecificHeatCapacityMeasure) obj)._theValue;
            else
                return 0.0;
        }
        ...
        public static explicit operator StepP21Token(IfcSpecificHeatCapacityMeasure? value)
        {
            if (value.HasValue)
                return new StepP21Token(((IfcSpecificHeatCapacityMeasure) value).ToString());
            else
