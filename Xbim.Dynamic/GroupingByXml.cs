...
namespace Xbim.Dynamic
{
    public class GroupingByXml
    {
        ...
        public bool GroupElements(string XMLfileName, IfcGroup rootGroup)
        {
            ...
            using (var trans = _model.BeginTransaction("Elements to groups"))
            {
        ...
        private bool PerformGrouping()
        {
            ...
            foreach (XmlNode group in groups)
            {
                ...
                foreach (XmlNode element in group)
                {
                    ...
                    IEnumerable<IPersistIfcEntity> instances = allElements.Where(qBuilder.BuildQuery().Compile());
        ...
        public Type GetIfcType(string typeName)
        {
            ...
            Type result = typeof(IfcProduct).Assembly.GetTypes().Where<Type>(t => t.Name.ToUpper() == typeName).FirstOrDefault();
            if (result == null)
