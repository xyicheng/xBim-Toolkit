...
namespace Xbim.Dynamic
{
    ...
    public class XbimQueryBuilder
    {
        ...
        {
            Type ifcType = GetIfcType(ifcTypeName);
            Init(ifcType);
        }

        private void Init(Type ifcType)
        {
            if (IfcTypes.Contains(ifcType))
            {
                _type = ifcType;
        ...
        /// <summary>
        /// Array of all the types contained in the assembly
        /// </summary>
        private Type[] IfcTypes
        {
            get { return typeof(IfcProduct).Assembly.GetTypes(); }
        }
        ...
        public static Type GetIfcType(string typeName)
        {
            typeName = typeName.ToUpper();
            Type[] types = typeof(IfcProduct).Assembly.GetTypes();
            return types.Where<Type>(t => t.Name.ToUpper() == typeName).FirstOrDefault();
        }
